...
def include_object(object_, name, type_, reflected, compare_to):
    """
    Filter what Alembic considers 'managed' by migrations.
    We ignore staging/scratch tables and selected legacy objects so
    autogenerate never proposes destructive drops for them.
    """
    # Ignore staging tables
    if type_ == "table" and name and name.startswith("stg_"):
        return False

    # Ignore legacy table that exists in DB but isn't modeled (keep as-is)
    if type_ == "table" and name == "assessment_answer_scale":
        return False

    # Ignore legacy indexes that exist in DB but may not be modeled
    if type_ == "index" and name in {
        "ux_answer_scale_version_active",
        "ux_explainability_v_l_key",
        "ux_question_translations_v_q_l",
        "ux_questions_version_code",
        "ux_skill_keyskill",
    }:
        return False
