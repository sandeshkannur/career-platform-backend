...
class AdminQuestionCreateRequest(BaseModel):
    ...
    question_id: str = Field(..., example="V1_Q3")
    skill_id: int = Field(..., example=1)

    # Minimal required text field (based on your current DB/table sample)
    question_text_en: str = Field(..., example="I like solving complex problems.")

    # Keep optional/future-proof fields without enforcing route logic
    question_text_hi: Optional[str] = Field(None, example="मुझे जटिल समस्याएँ हल करना पसंद है।")


class AdminQuestionCreateSuccess(BaseModel):
    status: str = Field("created", example="created")
    question_id: str = Field(..., example="V1_Q3")
    assessment_version: str = Field(..., example="v1")
    skill_id: int = Field(..., example=1)
...
class AdminQuestionCreateError(BaseModel):
    ...
    error_code: str = Field(..., example="ROW_VALIDATION_ERROR")
    message: str = Field(..., example="skill_id does not exist")
    field: Optional[str] = Field(None, example="skill_id")
