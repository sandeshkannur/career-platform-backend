# app/services/student_analytics_service.py
"""
Shared student drill-down analytics builder.

Extracted from app/routers/admin_student_analytics.py so that any endpoint
which has already established it's allowed to see a student's analytics
(admin, or a counsellor with an active assignment) can return the identical
7-panel + KPI-strip payload without duplicating the underlying queries.

Callers own: existence checks (404) and access enforcement. This module
only builds data.
"""
...
def get_student_row(db: Session, student_id: int):
    """Basic student identity row (id, name, grade, email, subscription_tier), or None."""
...
# ---------------------------------------------------------------------------
# Full analytics payload (7 panels + KPI summary)
# ---------------------------------------------------------------------------

def build_student_analytics(db: Session, student_id: int, student_row) -> dict:
    """
    Build the full drill-down analytics payload for a student.

    student_row must be the (already-fetched, non-None) result of
    get_student_row() — callers are responsible for the 404 check.
    """
    student = {
    ...
    try:
        ...
        section_a = []
        print(f"[student_analytics_service] section_a error student={student_id}: {e}")
    ...
    try:
        ...
        section_b_skill_profile = []
        print(f"[student_analytics_service] section_b_skill_profile error student={student_id}: {e}")
    ...
    try:
        ...
        section_b_platform_avg = []
        print(f"[student_analytics_service] section_b_platform_avg error: {e}")
    ...
    try:
        ...
        print(f"[student_analytics_service] section_c_frequency error student={student_id}: {e}")
    ...
    try:
        ...
        print(f"[student_analytics_service] section_c_rank1 error student={student_id}: {e}")
    ...
    try:
        ...
        section_d = []
        print(f"[student_analytics_service] section_d error student={student_id}: {e}")
    ...
    try:
        ...
        print(f"[student_analytics_service] section_f error student={student_id}: {e}")
    ...
    try:
        ...
        print(f"[student_analytics_service] section_g error student={student_id}: {e}")
